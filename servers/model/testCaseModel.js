        ...
        try {
            ...
            let query = `
               select a.APP_ID			as APP_ID
                    ...
                    , a.fsvc_cnt		as fsvc_cnt
                    , a.data_cnt		as data_cnt
                    , a.scnt			as scnt
                    , a.fcnt			as fcnt
                    , a.cmpCode			as cmpCode
                    , a.tenv			as tenv
    ...
    saveMessage: async (param) => {
        ...
        try {
            ...
            for (const item of inputList) {
                /*
                    // TC_ID 채번 (없거나 New인 경우)
                    if (!item.TC_ID || item.status === 'N') {
                        const rows = await conn.query(`
                                SELECT LPAD(IFNULL(MAX(CAST(SUBSTRING(TC_ID, 4) AS UNSIGNED)), 0) + 1, 11, '0') AS NEXT_SEQ 
                                FROM aqt_testcase_tb 
                                WHEREAPP_ID = ? 
                            `, [item.APP_ID]
                        );
                        
                        item.TC_ID = 'TCE' + rows[0].NEXT_SEQ;
                    }
                */
                //console.log("--------------------------------------------------------");
                //console.log("item.APP_ID : " + item.APP_ID);
                //console.log("item.TC_ID : " + item.TC_ID);
                //console.log("item.TC_NAME : " + item.TC_NAME);
                //console.log("item.TC_GUBUN : " + item.TC_GUBUN);
                //console.log("item.TC_WRITER : " + item.TC_WRITER);
                //console.log("item.TC_WRTDT : " + item.TC_WRTDT);
                //console.log("item.TC_BUSMGR : " + item.TC_BUSMGR);
                //console.log("item.TC_ITMGR : " + item.TC_ITMGR);
                //console.log("item.TC_SERVER : " + item.TC_SERVER);
                //console.log("item.TC_PORT : " + item.TC_PORT);
                ...
                const query = `
                    INSERT INTO aqt_testcase_tb (
                      ...
                      , scnt, fcnt, cmpCode, tenv
                        ...
                        TC_GUBUN = VALUES(TC_GUBUN),
                        TC_WRITER = VALUES(TC_WRITER),
                        TC_WRTDT = VALUES(TC_WRTDT),
                        TC_BUSMGR = VALUES(TC_BUSMGR),
                        TC_ITMGR = VALUES(TC_ITMGR),
                        TC_SERVER = VALUES(TC_SERVER),
                        TC_PORT = VALUES(TC_PORT),
                        UDT_ID = 'monadmin',
                        UDT_DT = SYSDATE(), 
                        type = VALUES(type),
                        ...
                        fsvc_cnt = VALUES(fsvc_cnt),
                        data_cnt = VALUES(data_cnt),
                        scnt = VALUES(scnt),
                        fcnt = VALUES(fcnt),
                        cmpCode = VALUES(cmpCode),
                        tenv = VALUES(tenv)
            ...
            await conn.commit();
            return { count: savedCount, message: "testCase saved successfully" };
