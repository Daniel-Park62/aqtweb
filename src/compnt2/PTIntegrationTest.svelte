<script>
    ...
    async function fetchTestcaseID() {
        try {
            const res = await fetch(
                `${$rooturl}/integrationTest/message/list?app_id=${searchAppId}`,
            );
    ...
    async function fetchTsellist() {
        try {
            const res = await fetch(
                `${$rooturl}/integrationTest/tsellist?tcode=${currentHeader.tcode}`,
            );
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    //상단 업무 선택시 상단 시나리오ID, 하단 그리드 자료조회 (업무 선택 선택하면 상단, 하단 내용 모두 초기화)
    ...
    async function handleTopSave() {
        if (!currentHeader.tcode) {
            alert("시나리오ID 는 필수입니다.");
            return;
        }

        if (!confirm("통합테스트 내용을 저장하시겠습니까?")) return;

        try {
            const res = await fetch(`${$rooturl}/integrationTest/save`, {
                method: "POST",
    ...
    async function fetchCommHeaders() {
        try {
            ...
            const res = await fetch(
                `${$rooturl}/integrationTest/list?${params.toString()}`,
            );
    ...
    async function handleGridDelete() {
        ...
        try {
            for (let id of selectedIds) {
                if (id) {
                    const res = await fetch(
                        `${$rooturl}/integrationTest/delete/${id}`,
                        {
                            method: "DELETE",
                        },
                    );
    ...
    function handleExcelDownload() {
        ...
        utils.book_append_sheet(wb, ws, "통합테스트");
        writeFile(wb, `통합테스트_${currentHeader.job}.xlsx`);
    }
    ...
    async function unitTestSelect(item) {
        ...
        currentHeader.tcode = item.tcode; //시나리오ID
    ...
    <div class="bg-white shadow border border-gray-300 p-4 mb-0">
        <div class="flex justify-end mb-2 space-x-2 items-center">
            <div class="flex space-x-2 mr-auto items-start">
                <h2 class="text-lg font-bold text-gray-700 px-5">통합테스트</h2>
            </div>
        ...
        >
            ...
            <div class="flex items-center space-x-1">
                ...
                >
                    <option value="">시나리오ID 선택</option>
                    {#each testcaseid as tcid}
                        <option value={tcid.SIO_ID}>{tcid.SIO_NM}</option>
                    {/each}
                </select>
            </div>
            <div class="flex items-center space-x-1">
