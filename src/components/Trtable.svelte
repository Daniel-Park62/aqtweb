...
<div class="flex flex-col h-full w-full border-0">
  ...
  <div
    class="flex-[1_1_0] h-full w-full overflow-y-auto [scrollbar-gutter:stable] p-2"
  >
        ...
        {:then rows}
          {#each rows as row, i (row.pkey)}
            ...
            >
              ...
              <td>{row.status}</td>
              <td class="text-right">{row.수신크기.toLocaleString("ko-KR")}</td>
              <td class="text-left"
                >{row.수신데이터 === null ? "" : row.수신데이터}</td
              >
              <td>{row.dstport}</td>
              <td>{row.appid}</td>
